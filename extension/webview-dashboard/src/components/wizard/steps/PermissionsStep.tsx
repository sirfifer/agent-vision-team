    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-vscode-muted">
          Select which operations Claude Code can perform autonomously. These are written to{' '}
          <code className="bg-vscode-widget-bg px-1 rounded">.claude/settings.local.json</code>.
        </p>
      </div>
      ...
      <div className="flex gap-4 text-sm">
        <div className="flex items-center gap-2">
          ...
          <span>
            Recommended: {recommendedCount}/{RECOMMENDED_PERMISSIONS.length}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-3 h-3 rounded bg-vscode-btn2-bg" />
          <span>
            Optional: {optionalCount}/{OPTIONAL_PERMISSIONS.length}
          </span>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        ...
        >
          {recommendedCount === RECOMMENDED_PERMISSIONS.length ? 'Deselect' : 'Select'} All
          Recommended
        </button>
        <button
          ...
          >
            ...
            <div className="space-y-2">
              {group.permissions.map((perm) => (
                <label key={perm.pattern} className="flex items-start gap-3 cursor-pointer group">
                  <input
                    type="checkbox"
      ...
      <div className="p-3 rounded bg-blue-500/10 border border-blue-500/30 text-sm">
        <strong>Note:</strong> These permissions allow Claude Code to run commands without asking
        for confirmation. You can always change them later in the settings panel or by editing{' '}

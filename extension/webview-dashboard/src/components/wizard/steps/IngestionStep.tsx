...
export function IngestionStep({ config, updateConfig }: IngestionStepProps) {
  ...
  const [archIngested, setArchIngested] = useState(
    config.ingestion.lastArchitectureIngest !== null,
  );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-vscode-muted">
          Ingest your vision and architecture documents into the Knowledge Graph. This makes them
          searchable and accessible to AI agents.
        </p>
      </div>
      ...
      <div className="p-4 rounded-lg border border-vscode-border bg-vscode-widget-bg">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-sm text-vscode-muted">
              {architectureDocs.length} document{architectureDocs.length !== 1 ? 's' : ''} in
              docs/architecture/
            </p>
          </div>
          <div className="flex items-center gap-2">
      ...
      {lastIngestionResult && (
        ...
        >
          <h4 className="font-medium mb-2">Last Ingestion: {lastIngestionResult.tier}</h4>
          <div className="text-sm space-y-1">
            ...
            {lastIngestionResult.errors.length > 0 && (
              <p className="text-amber-400">Errors: {lastIngestionResult.errors.join(', ')}</p>
            )}
          </div>
        </div>
      )}
      ...
      <div className="p-3 rounded bg-blue-500/10 border border-blue-500/30 text-sm">
        <strong>Note:</strong> Re-ingesting documents will replace existing entities in the
        Knowledge Graph. This is safe and recommended when documents have been updated.
      </div>
    </div>
  );
}

...
const DEFAULT_COMMANDS: Record<
  SupportedLanguage,
  { test: string; lint: string; build: string; format: string }
> = {
  python: {
    test: 'uv run pytest',
...
export function QualityConfigStep({
  config,
  updateConfig,
  updateSettings,
  updateQuality,
}: QualityConfigStepProps) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-vscode-muted">
          Select the languages used in your project and configure the commands for testing, linting,
          and building.
        </p>
      </div>
      ...
      <div>
        ...
        <div className="flex items-center gap-4">
          <input
            ...
            className="flex-1"
          />
          <span className="w-16 text-right font-mono">{config.settings.coverageThreshold}%</span>
        </div>

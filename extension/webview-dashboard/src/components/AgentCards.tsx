...
const roleDescriptions: Record<string, string> = {
  orchestrator:
    'Orchestrator: coordinates subagents, decomposes tasks, maintains the three-tier governance hierarchy',
  worker:
    'Worker: implements scoped task briefs, follows established patterns, submits decisions to governance',
  'quality-reviewer':
    'Quality Reviewer: evaluates work through vision alignment, architectural conformance, and quality compliance',
  'kg-librarian':
    'KG Librarian: curates institutional memory, consolidates observations, promotes solution patterns',
  'governance-reviewer':
    'Governance Reviewer: AI-powered review of decisions and plans against vision and architecture standards',
  researcher:
    'Researcher: gathers intelligence for architectural decisions and tracks external changes affecting the project',
  'project-steward':
    'Project Steward: maintains project hygiene, naming conventions, organization, and essential file completeness',
};
...
function statusBorderClass(status: string, isSelected: boolean): string {
  ...
  switch (status) {
    case 'blocked':
      return 'border-red-500/40 bg-red-500/5 hover:border-red-500/60';
    case 'reviewing':
      return 'border-amber-500/40 bg-amber-500/5 hover:border-amber-500/60';
    case 'active':
      return 'border-agent-active/40 bg-agent-active/5 hover:border-agent-active/60';
    default:
      return 'border-vscode-border bg-vscode-widget-bg hover:border-vscode-muted';
  }
}
...
function AgentCard({
  ...
  const filterHint = isSelected
    ? 'Click to clear activity filter'
    : 'Click to filter Activity Feed to this agent';
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 mb-0.5">
        ...
        {label && (
          <span
            className={`ml-auto text-2xs px-1 rounded shrink-0 ${
...
export function AgentCards() {
  ...
  if (agents.length === 0) {
    return (
      <div
        ...
        title="Agent cards appear here after connecting. Each card shows an agent's status and current task."
      >
        {Object.values(roleDisplayNames).map((name) => (
          <div
            key={name}
            className="flex-1 p-2 rounded border border-vscode-border bg-vscode-widget-bg opacity-50"
            title={`${name}: no data yet. Connect to MCP servers to detect agents.`}
          >
            <div className="flex items-center gap-1.5 mb-0.5">
  ...
  return (
    <div
      className="flex gap-2 px-4 py-2"
      title="Click an agent card to filter the Activity Feed to that agent's events"
    >
      {agents.map((agent) => (
        <AgentCard
          key={agent.id}

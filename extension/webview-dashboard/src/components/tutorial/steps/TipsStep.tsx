...
export function TipsStep() {
  return (
    <div className="space-y-6">
      ...
      <p className="text-vscode-muted">Practical advice for getting the most out of the system:</p>

      <div className="grid grid-cols-2 gap-3">
        {TIPS.map((tip) => (
          <div
            key={tip.title}
            className="p-3 rounded-lg border border-vscode-border bg-vscode-widget-bg/30 space-y-2"
          >
            <div className="flex items-center gap-2">
              <svg
                className="w-4 h-4 text-tier-quality flex-shrink-0"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >

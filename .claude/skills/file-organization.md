...
├── collab/
│   ├── knowledge-graph.jsonl        # KG persistence, JSONL backend (default)
│   ├── trust-engine.db              # Trust engine SQLite DB (default)
│   ├── governance.db                # Governance SQLite DB (default)
│   └── avt.db                       # Unified SurrealDB (when AVT_STORAGE_BACKEND=surreal)
...
├── design/                              # Design documents for significant decisions
│   ├── surrealdb-storage-backend.md     # SurrealDB unified persistence design
│   ├── audit-agent-design.md            # Audit agent architecture
│   ├── compatibility-monitor-design.md  # Compatibility monitor design
│   ├── context-drift-prevention.md      # Context drift prevention research
│   ├── intent-driven-architecture.md    # Intent-driven architecture protocol
│   └── project-bootstrapper-design.md   # Bootstrap protocol design
├── style/                               # Coding conventions and style guide
│   └── style-guide.md                   # Discovered conventions (generated by bootstrapper)
└── project-overview.md                  # Project overview (first-class external doc)
...
shared/                                  # Shared packages across MCP servers
└── avt_db/                              # Unified SurrealDB persistence layer
    ├── connection.py                    # Singleton connection manager (async + sync)
    ├── schema.py                        # 19-table schema (SCHEMA_VERSION=1)
    ├── migration.py                     # One-shot migration from JSONL/SQLite
    ...
    └── tests/                           # Foundation + vector search tests

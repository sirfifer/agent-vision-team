...
|-------|-----------|-------------|-----------|-------------|--------|
| Worker | Opus | `model: opus` | 9 | 9 (Read, Write, Edit, Bash, Glob, Grep, mcp:collab-kg, mcp:collab-quality, mcp:collab-governance) | CONFIRMED |
| Quality Reviewer | Opus | `model: opus` | 6 | 6 (Read, Glob, Grep, Bash, mcp:collab-kg, mcp:collab-quality) | CONFIRMED |
| KG Librarian | Sonnet | `model: sonnet` | 5 | 5 (Read, Write, Glob, Grep, mcp:collab-kg) | CONFIRMED |
| Governance Reviewer | Sonnet | `model: sonnet` | 4 | 4 (Read, Glob, Grep, mcp:collab-kg) | CONFIRMED |
| Researcher | Opus | `model: opus` | **7 (root doc) / 8 (Section 7 table)** | 7 (Read, Glob, Grep, WebSearch, WebFetch, mcp:collab-kg, mcp:collab-governance) | **INCONSISTENCY** -- Section 7 table says "8" but actual is 7 |
| Project Steward | Sonnet | `model: sonnet` | 7 | 7 (Read, Write, Edit, Glob, Grep, Bash, mcp:collab-kg) | CONFIRMED |
...
**Scenario count**: Doc claims 11 scenarios (s01-s10, s12, no s11). Verified: 11 scenario files exist (s01 through s10, s12). CONFIRMED.
...
**Assertion count**: Doc claims "172+ structural assertions." This was not independently verified by running the harness, but the doc is consistent in this claim across sections.

...
I'll just come right out and say it: this thing is over-engineered. I've pretty much settled into that fact, and I'm leading with it now, like so many other things I've been doing lately. This is not to say I'm surrendering to a whim, or not taking things seriously. It's the opposite. I think that since, with the aid of AI, I can effectively build this way, I can treat this more like a sculpture: subtractive, rather than additive, like 3D printing. Slightly weird analogy, but you get the point. Everything I'm doing these days, even when for a real purpose and for work, has an aspect of learning, of pushing the limits, of seeing what will work. Everything is moving so fast and I'm riding the wave.
...
This is the biggest addition to the system, and it started with someone else's work. Robin Lorenz published a LinkedIn post describing a 764-line PreToolUse hook he'd built for Claude Code that detects context drift by analyzing the model's extended thinking blocks and injecting relevant context via a 62-route JSON router. His core insight was that thinking blocks are the clearest signal of an agent's cognitive state, and that you can use keyword extraction from those blocks to match against a static set of context routes and inject corrections before problems manifest.

That idea landed hard. I'd been experiencing exactly the drift problem he was solving, and his approach proved it was tractable using Claude Code's existing hook infrastructure. I took his concept as the starting point and went deep on the research side to understand the problem more broadly and figure out how to adapt the approach for a multi-agent system with governance integration. The implementation I ended up with diverges significantly from his original (three injection layers instead of one, session context distillation, post-compaction recovery, governance pipeline integration), but the seed of the idea, using PreToolUse hooks with a context router to fight drift, that's Robin's contribution and I want to be clear about that.

The research I did after encountering his post was instructive. Chroma Research showed that 11 of 12 models drop below 50% of their short-context performance around 32,000 tokens. The "Lost in the Middle" paper from MIT found models perform roughly 30% worse on information placed in the middle of context versus at the beginning or end. And perhaps most usefully, Dongre et al. challenged the assumption that drift is an inevitable, unbounded decay. Their research suggests it's actually a bounded stochastic process with restoring forces: simple, targeted reminders can reliably reduce divergence.
...
Only when an anomaly is detected does the system escalate to AI analysis, and it does so through a tiered chain designed to minimize token usage and maximize speed. Haiku handles initial triage, fast and lightweight. Sonnet steps in for deeper analysis if warranted. Opus does a full deep dive only in serious cases. Each tier is a detached subprocess that spawns the next only if the previous tier determines escalation is needed. The vast majority of anomalies resolve at the Haiku tier, keeping the token footprint minimal and the latency negligible.

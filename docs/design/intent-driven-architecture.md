...
1. **Intent & Metrics Metadata** on every architectural entity
...
## Part 1: Intent & Metrics as First-Class Metadata

### The Problem

Today, architectural entities carry descriptions ("ServiceRegistry Pattern: decouple service creation from consumption") but not structured *intent* (why it exists) or *measurable outcome metrics* (what specific results it produces). Without these, governance can only say "you're breaking architecture" rather than engaging with whether the architecture is being well-served.
...
- Existing entities without intent metadata continue to function
...
3. For entities missing intent:
   - Claude proposes intent, metrics, and vision alignments based on the entity's description and available vision standards
...
2. Claude proposes intent and metrics based on the decision context
3. Human reviews and approves the promotion with enriched metadata
4. New architecture entity is created with full intent/metrics/vision-alignment
...
And proposes: intent, suggested metrics, vision alignments, and a confidence level. The prompt emphasizes: if the intent naturally demonstrates vision alignment, that is sufficient; do not manufacture artificial connections.
...
| **Covered, but improvable** | Agent believes it can better serve the intent | Evolution proposal workflow |
...
propose_evolution(target_entity, proposed_change, rationale, experiment_plan, validation_criteria)
    |
    v
Governance reviews proposal against intent/metrics/vision
...
| Tool | Purpose |
|------|---------|
| `propose_evolution(target_entity, proposed_change, rationale, experiment_plan, validation_criteria, agent)` | Submit an evolution proposal referencing the entity's intent and metrics |
...
| `mcp-servers/governance/collab_governance/reviewer.py` | Add `review_evolution_proposal()` method; add `_format_architecture_with_intent()` that surfaces intent/metrics in review prompts; update `_build_decision_prompt()` instructions to consider intent |
...
- **service_registry_pattern** (pattern): Decouple service creation from consumption
  ...
  Metrics: test_isolation_time (< 500ms per test, baseline: 847ms average)
...
3. If the decision is a "deviation" or "scope_change" category, verdict
   should be "needs_human_review".
4. If the decision aligns with standards and serves their intent, verdict
   is "approved".
5. **Intent-aware evaluation**: Architecture entities carry structured intent.
   Evaluate against INTENT, not just structure. If an entity has outcome
   metrics, check whether the decision maintains or improves those baselines.
```
...
- Intent and metrics can still be captured (they stand alone)
...
Two entities with conflicting intents or overlapping metrics:
- Detected during `validate_ingestion` by comparing intents and metrics
...
If an agent proposes evolution against an entity lacking intent metadata:
...
### E2E Testing
- **s14**: Create architecture entity with intent/metrics, verify `get_architecture_completeness` reports it correctly, verify `set_entity_metadata` updates it (36 assertions)
- **s15**: Ingest an architecture document containing Intent/Metrics/Vision Alignment sections, verify entities have structured metadata (36 assertions)
- **s16**: Full evolution lifecycle: propose, experiment, submit evidence, validate, approve; KGClient metadata helpers; query methods (29 assertions)
- **s17**: Cascading alignment: dependent entity discovery via serves_vision relations; evidence validator in non-mock mode (8 failure types); reviewer intent formatting (33 assertions)
- **s18**: Extended coverage: architecture promotion logic, cascade discovery pattern, evidence validator timestamp edge cases, error paths (invalid states, missing entities), intent-aware decision review prompt formatting (43 assertions)

Total: 177 structural assertions across 5 intent-driven architecture scenarios.
